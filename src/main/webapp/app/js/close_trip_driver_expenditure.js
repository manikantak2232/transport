...
/*function addDriverDropDownOptions(drivers){
  ...
  function getDrivers(){
	  trip_id=querystring('trip_id');
	  fk_driver_id=querystringDriver('fk_driver_id');
	  dispatch=querystringDispatch('dispatch');
	  
			var requestData = new Object();
			requestData.trip_id=trip_id;
			requestData.dispatch=dispatch;
			$.ajax({
			url: '/transport/trip/driver/expenditure/close/get',
			type: 'POST',
			dataType: 'json',
...
/*  function driverChange(){
  ...
  function querystringDispatch(key) {
	   var re=new RegExp('(?:\\?|&)'+key+'=(.*?)(?=&|$)','gi');
	   var r=[], m;
	   while ((m=re.exec(document.location.search)) != null) r.push(m[1]);
	   return r[0];
...
$(document).ready(function (){ 
	  $('#insert').click(function () {
		...
		$.ajax({
			url: '/transport/trip/driver/expenditure/add',
			type: 'POST',
			dataType: 'json',
			data: JSON.stringify(requestData),
			contentType: 'application/json; charset=utf-8',

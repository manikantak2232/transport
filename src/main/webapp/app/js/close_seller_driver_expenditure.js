...
/*function addDriverDropDownOptions(drivers){
  ...
  function getDrivers(){
	  seller_dispatch_id=querystring('seller_dispatch_id');
	  fk_driver_id=querystringDriver('fk_driver_id');
	  
			var requestData = new Object();
			requestData.seller_dispatch_id=seller_dispatch_id;
			$.ajax({
			url: '/transport/seller/driver/expenditure/close/get',
			type: 'POST',
			dataType: 'json',
...
$(document).ready(function (){ 
	  $('#insert').click(function () {
		...
		$.ajax({
			url: '/transport/seller/driver/expenditure/add',
			type: 'POST',
			dataType: 'json',
			data: JSON.stringify(requestData),
			contentType: 'application/json; charset=utf-8',

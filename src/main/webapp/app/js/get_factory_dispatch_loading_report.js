...
function getDispatch() {
	...
	xhr=$.ajax({	
		url: '/transport/factory/dispatch/report/loading/get',
		type: 'POST',
		...
		success: function(response) {
			...
			for(var i=0; i<svtcData.length; i++){
				...
				"<td width=5% style='text-align:center; padding-left:3px; padding-right:3px'> <input type='button' id='but' onclick='fun(\""+factory.dispatch_id+"\",\""+factory.company_name+"\",\""+factory.fk_association_id+"\")' value='update'></td>"+
				"<td width=5% style='text-align:center; padding-left:3px; padding-right:3px'> <input type='button' id='but' onclick='deleteFactoryDispatch(\""+factory.dispatch_id+"\")' value='del'></td>"+
			...
			for(var i=0; i<otherData.length; i++){
				...
				"<td width=5% style='text-align:center; padding-left:3px; padding-right:3px'> <input type='button' id='outside_but' onclick='fun2(\""+otherFactory.dispatch_id+"\",\""+otherFactory.company_name+"\",\""+otherFactory.fk_association_id+"\")' value='update'></td>"+
...
function fun(factory_dispatch_id,company_name,association_id){
	window.open("/app/views/update_factory_dispatch_report.html?"+
		"factory_dispatch_id="+factory_dispatch_id+"&company_name="+company_name+"&association_id="+association_id);
	
}

function fun2(factory_dispatch_id,company_name,association_id){
	window.open("/app/views/update_outside_factory_dispatch_report.html?"+
		"factory_dispatch_id="+factory_dispatch_id+"&company_name="+company_name+"&association_id="+association_id);
	
}

function deleteFactoryDispatch(dispatch_id){
...
//	alert(dispatch_id);
	...
	$.ajax({
		url: '/transport/factory/dispatch/delete',
		type: 'POST',
		dataType: 'json',
		data: JSON.stringify(requestData),
		contentType: 'application/json; charset=utf-8',

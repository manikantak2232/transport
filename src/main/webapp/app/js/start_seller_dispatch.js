 ...
 function assignDriver(){
   ...
   function getSellerDispatchedTrucks(){
 	  $(document).ready(function() {
 			  var truck_number;

 			  $.ajax({
 					url: '/transport/seller/trucks/dispatched/get',
 					type: 'GET',
 				   dataType: 'json',
...
function test(){  
	...
	if(check_truck_number==-1)
		...
		alert("select Truck Number only from Suggestions");
	        ...
	        $.ajax({
	            type: 'GET',
	            url: '/transport/seller/status/get?'+'truck_id='+truck_id,
	            dataType: 'json',
	            contentType: 'application/json; charset=utf-8',
	            success: function(response) {
	  ...
	  $(document).ready(function (){ 
			$('#update').click(function () {
				...
				$.ajax({
					url: '/transport/seller/status/update',
					type: 'POST',
					dataType: 'json',
